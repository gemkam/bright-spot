...
type Product = {
  ...
  name: string;
  note: string;
  price_pkr: string; // numeric comes back as string from the driver
      ...
      <div style={{ marginBottom: 16, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        {categories.map((c) => (
          <button
            ...
            style={filterBtnStyle(categoryFilter === c.id)}
          >
          ...
          {filtered.map((p) => (
            <tr key={p.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
              ...
              <td style={td}>
                ...
                />
                {p.stock_quantity === 0 && (
                  <span style={{ color: '#f87171', fontSize: 12, marginLeft: 6 }}>Out of stock</span>
                )}
                {p.stock_quantity > 0 && p.stock_quantity <= 5 && (
                  <span style={{ color: '#fbbf24', fontSize: 12, marginLeft: 6 }}>Low</span>
                )}
              </td>
              <td style={td}>
                <button
                  ...
                  style={{
                    ...
                    color: p.is_active ? '#4ade80' : 'rgba(255,255,255,0.5)',
                    cursor: 'pointer',
                  }}
                >
